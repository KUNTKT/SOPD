...
OUT=$BASE/reports/ssopd03_math_L14_nothink_confirm
CFG=$BASE/configs/experiment_ssopd03_math_L14_nothink_confirm.yaml
LOG=$OUT/supervise.log
MARKER='scale2k_paper_protocol_smoke_resume.py --config .*ssopd03_math_L14_nothink'
...
job_alive() {
  pgrep -f 'scale2k_paper_protocol_smoke_resume.py' >/dev/null && \
    pgrep -af 'scale2k_paper_protocol_smoke_resume.py' | grep -q 'ssopd03_math_L14_nothink'
}

start_job() {
  echo "[$(date -Is)] START/RESUME inject no-think" | tee -a "$LOG"
...
while true; do
  ...
  if ! job_alive; then
    ...
    tail -30 "$OUT/inject_nothink.log" 2>/dev/null | tee -a "$LOG"
    start_job
  fi
  last=$(grep -E 'audit rollout|PAPER SMOKE|confirm sweep|prompt_check|clean alpha' "$OUT/inject_nothink.log" 2>/dev/null | tail -1)
