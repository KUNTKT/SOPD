...
OUT=$BASE/reports/ssopd03_math_L14_nothink_negalpha
CFG=$BASE/configs/experiment_ssopd03_math_L14_nothink_negalpha.yaml
...
job_alive() {
  pgrep -af 'scale2k_paper_protocol_smoke_resume.py' | grep -q 'ssopd03_math_L14_nothink_negalpha'
}

start_job() {
  echo "[$(date -Is)] START/RESUME negalpha" | tee -a "$LOG"
...
while true; do
  ...
  if ! job_alive; then
    ...
    tail -20 "$OUT/inject_negalpha.log" 2>/dev/null | tee -a "$LOG"
    start_job
  fi
  last=$(grep -E 'audit rollout|PAPER SMOKE|confirm sweep|prompt_check|steered alpha' "$OUT/inject_negalpha.log" 2>/dev/null | tail -1)
