...
def patch_disable_thinking() -> None:
    """Qwen3 chat template defaults to <think>; injection reruns should turn it off."""
...
def main() -> None:
    ...
    ap.add_argument(
        "--no-thinking",
        action="store_true",
        help="Qwen3: apply_chat_template(enable_thinking=False).",
    )
    args = ap.parse_args()
    if args.no_thinking:
    ...
    log(
        ...
        f"resume={args.resume} no_thinking={args.no_thinking} out={reports_dir}"
    )
    ...
    sample_prompt = model.build_prompt(records[0].get("prompt_user", records[0]["problem"]))
    empty_think_prefill = "<think>\n\n</think>" in sample_prompt
    log(
        f"prompt_check no_thinking={args.no_thinking} empty_think_prefill={empty_think_prefill} "
        f"tail={sample_prompt[-160:]!r}"
    )
    if args.no_thinking and not empty_think_prefill:
        raise RuntimeError(
            "enable_thinking=False patch failed: expected Qwen3 empty <think></think> prefill"
        )
